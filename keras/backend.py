...
def _as_graph_element(obj):
    ...
    return conv_fn() if conv_fn and callable(conv_fn) else None
...
def is_placeholder(x):
    ...
    try:
        ...
        if not tf_utils.is_extension_type(x) or isinstance(x, tf.Variable):
...
def zeros(shape, dtype=None, name=None):
    ...
    with tf.init_scope():
        ...
        return variable(v, dtype=dtype, name=name) if py_all(v.shape.as_list()) else v
...
def batch_dot(x, y, axes=None):
    ...
    if x_ndim < 2 or y_ndim < 2:
        raise ValueError(
            f"Cannot do batch_dot on inputs with rank < 2. Received inputs with shapes {str(x_shape)} and {str(y_shape)}."
        )
    ...
    if x_batch_size is not None and y_batch_size is not None:
        if x_batch_size != y_batch_size:
            raise ValueError(
                f"Cannot do batch_dot on inputs with different batch sizes. Received inputs with shapes {str(x_shape)} and {str(y_shape)}."
            )
    ...
    if d1 is not None and d2 is not None and d1 != d2:
        raise ValueError(
            f"Cannot do batch_dot on inputs with shapes {str(x_shape)} and {str(y_shape)} with axes={axes}"
...
def normalize_batch_in_training(x, gamma, beta, reduction_axes, epsilon=1e-3):
    ...
    if ndim(x) != 4 or list(reduction_axes) not in [[0, 1, 2], [0, 2, 3]]:
        return (
            ...
            else _broadcast_normalize_batch_in_training(
                x, gamma, beta, reduction_axes, epsilon=epsilon
            )
        )
...
class GraphExecutionFunction:
    ...
    def __init__(
        ...
        if session_kwargs:
            raise ValueError(
                f"Some keys in session_kwargs are not supported at this time: {session_kwargs.keys()}"
            )
